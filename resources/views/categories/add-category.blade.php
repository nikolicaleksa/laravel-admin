...
@section('content')
    <div class="container">
        ...
        <form id="add-category-form" class="form-horizontal" method="POST" action="{{ route('addCategory') }}" role="form">
            @csrf
            <div class="form-group">
                <label for="name" class="col-xs-2 control-label">@lang('content.categories.name')</label>
                <div id="name_div" class="col-xs-10">
                    <input type="text" name="name" id="name" class="form-control" placeholder="@lang('content.categories.name')" tabindex="1">
